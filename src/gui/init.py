...
def init_server_view():
    ...
    #Connect button
    connect_button = tk.Button(frame, text='Connect', command=callbacks.connect_to_server, **config.BUTTON_THEME)
    component = components.Component(connect_button, sticky='NSEW', row=0, column=2)
    view.add_component(component, 'connect_button')

    #Host button
    host_button = tk.Button(frame, text='Host', command=callbacks.host_server, **config.BUTTON_THEME)
    ...
    component = components.Component(username_label, sticky='NSE', row=1, column=0, padx=5)
    ...
    #update button
    update_button = tk.Button(frame, text='Update', command=callbacks.update_username, **config.BUTTON_THEME)
...
def init_status_view():
    ...
    status_entry = tk.Entry(frame, textvariable=app.data['status'], **config.STATUS_THEME)
